...
<script src="https://cdn.jsdelivr.net/npm/jquery@3.6.3/dist/jquery.min.js"></script>
  ...
  <title> Attendance Marker </title>
    ...
    <div class="meetings flex flex-row gap-32 ">
  ...
  session_start();
   ...
   include 'dashboardNav.php';
    ...
    ?>
  <div class="meeting_list h-fit grid grid-cols-3 md:grid-cols-2  gap-10 ml-2 mt-2 mr-2">
  <a href="./Meeting_form.php" style="padding-top:12em;">
    <button class="meeting text-[#512bd4]  w-fit flex flex-col mt-[12em] mx-auto" type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalForm">

  <i class="fa-solid fa-plus text-4xl"></i>
  <p class="font-extrabold font-[Mulish] text-2xl -indent-10 mt-3  ">Add Meeting</p>
  </button>
  </a>
...
<?php
  ...
  $db = new Db;
   $meetings = $db->getCoordinatingMeetings($_SESSION['name']);
    if ($meetings !== 'You are coordinating 0 Meeting') {
  ...
  foreach ($meetings as  $meeting) {
    ...
    echo  '<p class="absolute" id="startdate" style="opacity: 0;" >'.    
        ...
        date_format(new DateTime($meeting["Meeting_Startdate"].' '.$meeting["Meeting_Time"]),"Y/m/d H:i:s")
  .'</p>';
  
    echo  '<p class="absolute" id="enddate" style="opacity: 0;" >'.
    
    date_format(new DateTime($meeting["Meeting_Enddate"]),"Y/m/d H:i:s").'</p>';
    echo  '<p class="absolute" id="classTaken" style="opacity: 0;" >'.
    $meeting["Meeting_Taken"].'</p>';
    echo  '<p class="absolute" id="noc" style="opacity: 0;" >'.
    
    $meeting["No_Of_Meetings"].'</p>';

    echo '<form id="marker" class="h-fit"   action="./students_assessments.php" method="GET">
    <button id="marker_link" type="submit">
    <div class="meeting border w-fit flex flex-col   px-20 h-fit   py-16 rounded-md ">
    ...
    <input readonly name="meeting" class=" font-[Montserrat]  font-extrabold ml-2 text-[#939ca5]" value='.$meeting['Meeting_Code'].'>
    </div>
...
';
  ...
  echo '<p class="text-3xl>You are coordinating 0 Meeting </p>';
}
?>
  ...
  let no_of_meetings = document.getElementById("noc").innerText
  let percentage = (classTaken/ no_of_meetings) * 100
